...
UniValue prioritisetransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_GreenLeaves / txsize) \n"
            "3. fee_delta      (numeric, required) The fee value (in GreenLeaves) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                    (numeric) difference in value between transaction inputs and outputs (in GreenLeaves); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "  },\n"
            "  \"coinbasevalue\" : n,              (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in GreenLeaves)\n"
